...
function getFidEntries(geojson) {
    ...
    for (const feature of features) {
        ...
        const typeLabel = type.includes("puit") ? "Puit" : "Lavoir/Fontaine";
        
        entries.push({
            fid: fid,
            label: `${fid} - ${name} (${typeLabel})`
        });
    }

    // Sort by fid (as number) and return array of string labels
    return entries
...
function buildIssueTemplate(entries) {
    const optionsYaml = entries
        .map((label) => `        - '${escapeYamlSingleQuoted(label)}'`)
        .join("\n");
